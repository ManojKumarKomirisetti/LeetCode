...
class MinimumBitFlipsToConvertNumber {
    public int minBitFlips(int start, int goal) {
        int ans = 0;      // Initialize counter for number of flips.
        int n = start ^ goal;  // XOR to find different bits.

        // Count the number of set bits (1s) in the XOR result.
        while (n > 0) {
            // Check if the least significant bit is 1.
            if ((n & 1) == 1) {
                ans++;   // Increment the count if the bit is 1.
            }
            // Shift the number to the right to process the next bit.
            n = n >> 1;
        }

        return ans;  // Return the total number of bit flips.
    }

    public static void main(String[] args) {
        // Example: passing arguments start and goal
